...
class LabAgentApp {
    ...
    bindEvents() {
        ...
        if (this.quickUploadBtn && this.fileInput) {
    ...
    renderChatHistory() {
        ...
        this.chatHistories.forEach((history, index) => {
            ...
            const timeStr = this.formatHistoryTime(history.updatedAt || history.createdAt);
            const timeHtml = timeStr
                ? `<span class="history-item-time">${this.escapeHtml(timeStr)}</span>`
                : '';
                ...
                <div class="history-item-content">
                    <span class="history-item-icon" aria-hidden="true">
                        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <path d="M6 4h12a2 2 0 0 1 2 2v14l-4-3H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/>
                        </svg>
                    </span>
    ...
    updateUI() {
        ...
        if (this.messageInput) {
            ...
            this.messageInput.placeholder = '向 Lab Agent 提问…';
        }
    }
                                    ...
                                    for (const jsonStr of matches) {
                                        try {
                                            ...
                                            if (sseMessage.type === 'content') {
                                                ...
                                                throw new Error(sseMessage.data || sseMessage.message || '运维分析失败');
                                            }
                                        } catch (e) {
                                            if (e.message.includes('运维分析')) throw e;
    ...
    async triggerAIOps() {
        ...
        try {
            ...
            if (loadingMessage) {
                ...
                if (messageContent) {
                    messageContent.textContent = '抱歉，AI Ops 分析出现问题：' + error.message;
                }
            }
        } finally {
            this.isStreaming = false;
    ...
    showLoadingOverlay(show) {
        if (this.loadingOverlay) {
            if (show) {
                ...
                if (loadingText) loadingText.textContent = '分析进行中，请稍候…';
                if (loadingSubtext) loadingSubtext.textContent = '后端正在处理，请稍候';
                // 防止页面滚动
