...
# 创建并编辑本地配置，至少填写 DASHSCOPE_API_KEY
cp .env.example .env
...
应用默认从项目根目录的 `.env` 读取配置。可复制 [`.env.example`](.env.example) 后填写密钥：
...
![Lab Agent 演示闭环](docs/demo-flow.png)
...
├── docs/
│   ├── demo-flow.svg            # 面试展示用端到端演示闭环图源文件
│   └── demo-flow.png            # README 展示用端到端演示闭环图
